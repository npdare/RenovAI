      ...
      try {
        if (!req.file) {
          return res
            .status(400)
            .json({
              error: "Photo file is required for architectural analysis",
            });
        }
        ...
        res
          .status(500)
          .json({ error: "Failed to analyze architectural elements" });
      }
    },
  );
      ...
      try {
        ...
        if (req.body.architecturalElements) {
          try {
            ...
            const transformElements = elements.filter(
              (el: any) =>
                el.action === "inspiration" || el.action === "select",
            );
            architecturalContext = transformElements
              .map((el: any) => `${el.category}: ${el.specificType}`)
              .join(", ");
          ...
          messages: [
            {
              role: "system",
              content:
                "You are an expert design consultant. Extract inspiration elements that align with specific architectural elements from the original photo. Create dynamic categories based on what's being transformed.",
            },
            {
              role: "user",
              content: [
...
Return JSON:
{
  ...
  "dynamicCategories": [
    {
      ...
      "confidence": 0.9
        ...
        const dynamicParameters = {
          ...
          spaceType:
            analysisResult.spaceType || ("interior" as "interior" | "exterior"),
    ...
    async (req: any, res: Response) => {
      try {
        if (!req.file) {
          return res
            .status(400)
            .json({ error: "Photo file is required for preprocessing" });
        }
  ...
  app.post(
    "/api/v2/architectural-analysis",
    async (req: Request, res: Response) => {
      try {
        const { jobId, maskURIs, maskInfo } = req.body;

        if (!jobId || !maskURIs) {
          return res
            .status(400)
            .json({ error: "Job ID and mask URIs are required" });
        }

        const { analyzeArchitecturalElementsV2 } = await import(
          "./ai-service-v2"
        );
        const analysis = await analyzeArchitecturalElementsV2(
          jobId,
          maskURIs,
          maskInfo,
        );
  ...
  app.post("/api/v2/transform-image", async (req: Request, res: Response) => {
    try {
      const { transformImageV2, saveJobMetadata } = await import(
        "./ai-service-v2"
      );
  ...
  app.post(
    ...
    async (req: any, res: Response) => {
      try {
        ...
        const { parameters: paramString, transformationStrength = 75 } =
          req.body;
